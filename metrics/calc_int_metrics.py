...
def compute_metrics(latent_adata, adata, model, dataset, batch_key='study', label_key='cell_type'):
    ...
    bio_con = 0.0
    bio_metrics = ['NMI_cluster/label', 'ARI_cluster/label', 'ASW_label','isolated_label_F1', 'isolated_label_silhouette', 'KNN']
...
for model in models:
    for experiment in experiments:
        ...
        for dataset in datasets:
            if dataset == 'pbmc':
                adata = sc.read(os.path.expanduser(f'~/Documents/benchmarking_datasets/benchmark_pbmc_shrinked.h5ad'))
                batch_key = 'condition'
            elif dataset == 'brain':
                adata = sc.read(os.path.expanduser(f'~/Documents/benchmarking_datasets/benchmark_brain_shrinked.h5ad'))
                batch_key = "study"
                ...
                adata = sc.read(os.path.expanduser(f'~/Documents/benchmarking_datasets/benchmark_lung_shrinked.h5ad'))
                batch_key = "condition"
            elif dataset == "tumor":
            ...
            print(dataset)
            latent_adata = sc.read(os.path.expanduser(
            ...
            latent_adata.obs[batch_key] = latent_adata.obs[batch_key].astype(dtype='float64')
            latent_adata.obs[label_key] = latent_adata.obs[label_key].astype(dtype='float64')

            df = compute_metrics(latent_adata, adata, model, dataset, batch_key, label_key)
            scores = pd.concat([scores, df], axis=0) if scores is not None else df
        scores.to_csv(os.path.expanduser(f'~/Documents/{model}_benchmarks/batchwise/{experiment}/{model}_results.csv'), index=False)
