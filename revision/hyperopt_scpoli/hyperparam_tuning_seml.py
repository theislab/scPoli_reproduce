...
def run(
    ...
    logging.info(
        f"Dataset: {data}, latent_dim: {latent_dim},"
    ...
    REF_PATH = f"/lustre/groups/ml01/workspace/carlo.dedonno/lataq_reproduce_backup/lataq_reproduce/tmp/ref_model_embedcvae_{overwrite}"
    RES_PATH = (
        f"/lustre/groups/ml01/workspace/carlo.dedonno/lataq_reproduce_backup/lataq_reproduce/results/hyperopt"
    )
    ...
    results_dict = tranvae_query.classify(
        adata.X, adata.obs[condition_key], metric='dist'
    )
    ...
    results_dict_query = tranvae_query.classify(
        target_adata.X, target_adata.obs[condition_key], metric='dist'
    )
    for i in range(len(cell_type_key)):
    ...
    plt.savefig(
        f"{RES_PATH}/condition_umap_{model}_{data}_{latent_dim}_{hidden_layers}.png",
        bbox_inches="tight",
    )
    plt.close()
    sc.pl.umap(adata_latent, color=cell_type_key[0], show=False, frameon=False)
    plt.savefig(
        f"{RES_PATH}/ct_umap_{model}_{data}_{latent_dim}_{hidden_layers}.png",
        bbox_inches="tight",
    )
