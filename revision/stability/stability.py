...
def run(
    ...
    if n_obs > len(source_adata):
        source_adata = source_adata.concatenate([source_adata]*4)
    source_adata = sc.pp.subsample(source_adata, n_obs=n_obs, random_state=42, copy=True)
    ...
    plt.savefig(
        f"{RES_PATH}/condition_umap_{data}_{n_obs}.png",
        bbox_inches="tight",
    )
    plt.close()
    sc.pl.umap(adata_latent, color=cell_type_key[0], show=False, frameon=False)
    plt.savefig(
        f"{RES_PATH}/ct_umap_{data}_{n_obs}.png.png",
        bbox_inches="tight",
    )
