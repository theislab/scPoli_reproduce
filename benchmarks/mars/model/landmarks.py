...
def compute_landmarks_tr(embeddings, target, prev_landmarks=None, tau=0.2):
    ...
    landmarks_mean = torch.stack(
        [embeddings[idx_class].mean(0) for idx_class in class_idxs]
    ).squeeze()
    ...
    lndmk_dist_part = (tau / (nlndmk - 1)) * torch.stack(
        [suma - p for p in prev_landmarks]
    )
...
def init_landmarks(
    n_clusters, tr_load, test_load, model, device, mode="kmeans", pretrain=True
):
    ...
    lndmk_tr = [
        torch.zeros(
            size=(len(np.unique(dl.dataset.y)), model.z_dim),
            requires_grad=True,
            device=device,
        )
        for dl in tr_load
    ...
    lndmk_test = [
        torch.zeros(size=(1, model.z_dim), requires_grad=True, device=device)
        for _ in range(n_clusters)
    ]
    kmeans_init_tr = [
        init_step(dl.dataset, model, device, pretrained=pretrain, mode=mode)
        for dl in tr_load
    ]
    kmeans_init_test = init_step(
        test_load.dataset,
        model,
        device,
        pretrained=pretrain,
    ...
    with torch.no_grad():
        ...
        [
            lndmk_test[i].copy_(kmeans_init_test[i, :])
            for i in range(kmeans_init_test.shape[0])
        ]
...
def init_step(dataset, model, device, pretrained, mode="kmeans", n_clusters=None):
    ...
    if mode == "kmeans++":
        if not pretrained:  # find centroids in original space
            landmarks = k_means_._init_centroids(
                X.cpu().numpy(), n_clusters, "k-means++"
            )
            ...
            encoded, _ = model(X)
            landmarks = k_means_._init_centroids(
                encoded.data.cpu().numpy(), n_clusters, "k-means++"
            )
