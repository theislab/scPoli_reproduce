...
class MARS:
    def __init__(
        ...
        """
        train_load, test_load, pretrain_load, val_load = init_data_loaders(
            labeled_data,
            unlabeled_data,
            pretrain_data,
    ...
    def train(self, evaluation_mode=True, save_all_embeddings=True):
        ...
        optim_pretrain = torch.optim.Adam(
            params=list(self.model.parameters()), lr=self.lr
        )
        ...
        landmk_tr, landmk_test = init_landmarks(
            self.n_clusters,
            self.train_loader,
            self.test_loader,
            self.model,
        ...
        optim, optim_landmk_test = self.init_optim(
            list(self.model.encoder.parameters()), landmk_test, self.lr
        )
        ...
        for epoch in range(1, self.epochs + 1):
            self.model.train()
            loss_tr, acc_tr, landmk_tr, landmk_test = self.do_epoch(
                tr_iter, test_iter, optim, optim_landmk_test, landmk_tr, landmk_test
            )
            ...
            with torch.no_grad():
                ...
                postfix = (
                    " (Best)" if acc_val >= best_acc else " (Best: {})".format(best_acc)
                )
    ...
    def save_result(self, tr_iter, adata_test, save_all_embeddings):
        ...
        adata.obsm["MARS_embedding"] = np.concatenate(
            [a.uns["MARS_embedding"] for a in adata_all]
        )
    ...
    def assign_labels(self, landmk_test, evaluation_mode):
        ...
        x_test, y_true, cells = next(
            test_iter
        )  # cells are needed because dataset is in random order
    ...
    def do_epoch(
        self, tr_iter, test_iter, optim, optim_landmk_test, landmk_tr, landmk_test
    ):
        """
        One training epoch.
        ...
        for task in task_idx:
            ...
            x, y = x.to(self.device), y.to(self.device)
            encoded, _ = self.model(x)
            curr_landmk_tr = compute_landmarks_tr(
                encoded, y, landmk_tr[task], tau=self.tau
            )
        ...
        x = x.to(self.device)
        encoded, _ = self.model(x)
        loss, args_count = loss_test(
            encoded, torch.stack(landmk_test).squeeze(), self.tau
        )
    ...
    def name_cell_types(
        ...
        for idx, exp in enumerate(experiments[:-1]):
            ...
            if exp == self.unlabeled_metadata:
                raise ValueError(
                    "Error: Unlabeled dataset needs to be last one in the input anndata object."
                )
        ...
        for ytest in uniq_ytest:
            ...
            prob = np.exp(
                -np.power(distance.cdist(mean, landmk_tr, metric="euclidean"), 2)
                / (2 * sigma * sigma)
            )
            ...
            for cell_type in uniq_tr:  # sum probabilities of same landmarks
                prob_unique.append(
                    np.sum(prob[np.where(landmk_tr_labels == cell_type)])
                )
