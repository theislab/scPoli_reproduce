...
def run(
    ...
    plt.savefig(f"{RES_PATH}/query_uncertainty.png", bbox_inches="tight")
    ...
    adata_latent.write_h5ad(f"{RES_PATH}/adata_latent_query.h5ad")

    sc.pp.neighbors(adata_latent)
    sc.tl.leiden(adata_latent)
    sc.tl.umap(adata_latent)
    sc.pl.umap(adata_latent, color=["batch"], frameon=False, wspace=0.6, show=False)
    plt.savefig(f"{RES_PATH}/query_umap_batch.png", bbox_inches="tight")
    plt.close()
    sc.pl.umap(adata_latent, color=["celltype"], frameon=False, wspace=0.6, show=False)
    plt.savefig(f"{RES_PATH}_query_umap_ct.png", bbox_inches="tight")
    plt.close()
    sc.pl.umap(
        adata_latent, color=["predictions"], frameon=False, wspace=0.6, show=False
    )
    plt.savefig(f"{RES_PATH}/query_umap_pred.png", bbox_inches="tight")
    plt.close()
    sc.pl.umap(adata_latent, color=["checking"], frameon=False, wspace=0.6, show=False)
    plt.savefig(f"{RES_PATH}/query_umap_checks.png", bbox_inches="tight")
    plt.close()
    ...
    report_full = pd.DataFrame(
        classification_report(
            y_true=adata_full.obs[cell_type_key[0]], y_pred=preds, output_dict=True
        )
    ).transpose()
    ...
    ax.violinplot(data)
    labels = ["Correct", "Incorrect"]
    plt.savefig(f"{RES_PATH}/full_uncertainty.png", bbox_inches="tight")
    ...
    plt.savefig(f"{RES_PATH}/full_umap_query.png", bbox_inches="tight")
    plt.close()
    sc.pl.umap(adata_latent, color=["celltype"], frameon=False, wspace=0.6, show=False)
    ...
    plt.close()
    sc.pl.umap(adata_latent, color=["checking"], frameon=False, wspace=0.6, show=False)
    plt.savefig(f"{RES_PATH}/full_umap_checks.png", bbox_inches="tight")
    plt.close()
